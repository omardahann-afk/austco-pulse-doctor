...
export const Route = createFileRoute("/api/live-capture/$id/mark-reproduction-finished")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        try {
          const body = await request.text();
          const upstream = await fetch(`${BACKEND}/api/live-capture/${encodeURIComponent(params.id)}/mark-reproduction-finished`, {
            method: "POST",
