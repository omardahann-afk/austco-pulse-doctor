...
export const Route = createFileRoute("/api/live-capture/$id/ingest-logs")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        try {
          const body = await request.text();
          const upstream = await fetch(`${BACKEND}/api/live-capture/${encodeURIComponent(params.id)}/ingest-logs`, {
            method: "POST",
