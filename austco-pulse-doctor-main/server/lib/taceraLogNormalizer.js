...
const RULES = [
  ...
  // -------------------- Austco/Tacera field logic --------------------
  { service: "*", re: /(low bus voltage|bus voltage.*(low|fault))/i,
    eventType: "CONTROLLER_LOW_BUS_VOLTAGE", severity: "critical",
    normalizedMeaning: "Controller is reporting low bus voltage — field power/wiring issue, not server software",
    relatedServices: ["controller", "field-wiring", "power"],
    correlationTags: ["controller", "bus_voltage", "field_power"],
    suggestedTechCheck: "Measure voltage at controller and inspect field bus wiring/load before touching IPConnect/Pulse.",
    doNotDo: ["Do not restart IPConnect.", "Do not restart Pulse Gateway.", "Do not restart INGA."],
    rootCauseHint: "controller-field-power", delta: 25 },
  { service: "*", re: /(ip[- ]?in8|input).*\b(active|stuck|held)\b|door.*(active|alarm|held)|access control.*(active|alarm)/i,
    eventType: "IP_IN8_INPUT_ACTIVE", severity: "warning",
    normalizedMeaning: "Input module is receiving an active external signal; nurse call is likely responding correctly",
    relatedServices: ["controller", "ip-in8", "access-control"],
    correlationTags: ["ip-in8", "input_active", "access_control"],
    suggestedTechCheck: "Verify the upstream access-control relay/contact is releasing before changing nurse-call logic.",
    doNotDo: ["Do not change nurse-call logic before verifying the input source.", "Do not restart Pulse Gateway."],
    rootCauseHint: "external-input-active", delta: 22 },
  { service: "*", re: /(bed pad|bedpad).*(activat|cancel|repeat|bounce|flap)/i,
    eventType: "BED_PAD_REPEATED_ACTIVATION", severity: "warning",
    normalizedMeaning: "Integrated bed pad/input is repeatedly activating/cancelling, indicating unstable pad/cable/input source",
    relatedServices: ["controller", "bed-pad", "callpoint"],
    correlationTags: ["bed_pad", "input_flap"],
    suggestedTechCheck: "Inspect/replace the integrated bed pad and cable before changing software configuration.",
    doNotDo: ["Do not restart server-side services for a flapping bed pad."],
    rootCauseHint: "bed-pad-input-fault", delta: 22 },
  { service: "*", re: /(emergency).*\b(staff assist|wrong color|wrong colour|wrong tone|tone|colour|color)\b|staff assist.*emergency/i,
    eventType: "CALL_TYPE_MAPPING_MISMATCH", severity: "warning",
    normalizedMeaning: "Emergency call behavior looks mapped as Staff Assist or has wrong color/tone profile",
    relatedServices: ["ipconnect", "display", "call-routing"],
    correlationTags: ["call_type", "emergency", "staff_assist", "tone_color"],
    suggestedTechCheck: "Compare the affected section against a working room/floor and fix call type/color/tone mapping.",
    doNotDo: ["Do not assume speaker/display hardware failure until call-type/tone config is checked."],
    rootCauseHint: "call-type-config-mismatch", delta: 24 },
  { service: "*", re: /(tone filter|filter scheme|tone scheme|no chime|not chiming|all tones).*\b(missing|not selected|none|disabled)\b/i,
    eventType: "TONE_FILTER_SCHEME_MISSING", severity: "warning",
    normalizedMeaning: "Tone/chime behavior points to missing or unselected filter scheme, not necessarily speaker failure",
    relatedServices: ["display", "tone", "zts", "ipconnect"],
    correlationTags: ["tone", "filter_scheme", "display"],
    suggestedTechCheck: "Verify the tone/filter scheme is selected for the ZTS/display route before checking hardware.",
    doNotDo: ["Do not replace speakers/sound card until filter scheme is confirmed."],
    rootCauseHint: "tone-filter-config", delta: 20 },
  { service: "*", re: /(rtls|badge).*(cancel).*\b(same badge|same source|not cancel|only cancel)\b/i,
    eventType: "RTLS_BADGE_CANCEL_SOURCE_MATCH", severity: "info",
    normalizedMeaning: "RTLS badge cancel only cancels calls generated from that same badge/source",
    relatedServices: ["rtls", "ipconnect", "call-cancel"],
    correlationTags: ["rtls", "badge_cancel", "source_match"],
    suggestedTechCheck: "Confirm whether the call was created by the same badge before treating cancel as failed.",
    doNotDo: ["Do not troubleshoot this as a global cancel failure if the source badge differs."],
    rootCauseHint: "rtls-cancel-source-scope", delta: 18 },
  { service: "*", re: /(rtls|presence|room resolver|room association|badge room).*(missing|not resolving|generic floor|wrong room|unknown room)/i,
    eventType: "RTLS_ROOM_RESOLUTION_FAILED", severity: "warning",
    normalizedMeaning: "RTLS event received but room association/resolver mapping is incomplete or wrong",
    relatedServices: ["rtls", "room-mapping", "ipconnect"],
    correlationTags: ["rtls", "room_resolver", "mapping"],
    suggestedTechCheck: "Verify RTLS modules behind smart call points, room resolver mapping and badge room association.",
    doNotDo: ["Do not restart IPConnect before proving RTLS events/mapping are present."],
    rootCauseHint: "rtls-room-mapping", delta: 22 },
];
...
export function normalizeLogLines({ deviceProfile = null, deviceId = null, lines = [], sourcePath = null } = {}) {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    events.push({
      ...
      doNotDo: Array.isArray(rule.doNotDo) ? rule.doNotDo.slice() : [],
      line: i + 1,
    });
  }
